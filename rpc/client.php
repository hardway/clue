...
class Client{
    ...
    function __call($name, $arguments){
        ...
        $this->debug_log("[RPC] Payload: ".json_encode($payload));
        ...
        if($this->cache_dir){
            ...
            $this->debug_log("[RPC] Checking Cache: $cache_file");

            if(is_file($cache_file) && time()-filemtime($cache_file)<$this->cache_ttl){
                $this->debug_log("[RPC] Using Cache: $cache_file");
...
redirect:
        ...
        if($this->secret){
            $this->debug_log(sprintf("[RPC] ENCRYPTED RESPONSE:\n====================\n%s\n\n", $response));
            $response=clue_rpc_decrypt($response, $this->secret);
        }

        $this->bookkeep($record+['status'=>200, 'response'=>$response]);

        $this->debug_log(sprintf("[RPC] RESPONSE:\n====================\n%s\n\n", $response));

        if($this->cache_dir){
            $this->debug_log("[RPC] Writing Cache: $cache_file");
