...
END;
        ...
        function handle($argv){
            ...
            foreach($args as $a){
                ...
                elseif(strpos($a, '-')===0 && !is_numeric($a[1])){
                    if(strpos($a, '=')>0){
            ...
            if($is_piped){
                ...
                while(!feof($pipe)){
                    $batch=fgetcsv($pipe, 0, ",");
                    if(empty($batch)) break;

                    $batch=array_filter(array_map('trim', $batch), 'strlen');

                    $this->run_command($func, $help, $params+$batch, $options);
                }
            }
            else{
                // 单行命令
                return $this->run_command($func, $help, $params, $options);
            }
        }

        function run_command($func, $help, $params, $options){
            // 检查parameter不足，则显示帮助信息
            if(count($params) < count($help['param'])){
                $this->help_command($func);
                exit(-1);
            }
