<?php  
    ...
    class Test_Route extends UnitTestCase{
        ...
        function test_reform_with_lack_of_params(){
            ...
            $this->expectException(new Exception("COUND NOT REFORM"));
        ...
        function test_reform_rule_order(){
            $R=new Clue_RouteMap();
            $R->connect('/:code/:id/:action', array('controller'=>'message'));
            $R->connect(':controller/:action');
            
            $url=$R->reform('message', 'remove');
            $this->assertEqual($url, 'message/remove');
